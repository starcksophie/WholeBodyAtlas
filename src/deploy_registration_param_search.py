...
from reg_utils import load_config, register_deformable, register_affine
from data_utils import load_and_preprocess
from metrics import dice, NMILOSS, calculate_jacobian_metrics
...
def pairwise(fixed, warped, fixed_seg, warped_seg, transform):
    ...
    metrics = {'psnr': PSNR(fixed, warped).item(),
...
if  __name__ == '__main__':
    ...
    if not moving_path.exists():
        ...
        # affine reg
        _, transform, warped, warped_label = register_affine(ref, source_path=PATH_DATA.joinpath(str(eid), 'wat.nii.gz'),
                                                            source_seg=PATH_SEG.joinpath(str(eid), f'{str(eid)}_total_seg.nii.gz'),
                                                            target_grid=target_grid,
                                                            iterations=200,
                                                            save = (base_path, sitk_arr),
                                                            device=device)

    warped, warped_seg, deformable_trf = register_deformable(fixed_path,
                                                                moving_path,
                                                                deformable_config,
                                                                'cuda',
                                                                source_seg=moving_seg_path,
